...
protocol VehicleModelDelegate: class {
    var isFilerting: Bool {get}
    func dataUpdated()
}

class VehicleModel {
    weak var delegate: VehicleModelDelegate?
    ...
    var selectedVehicle: Vehicle {return delegate?.isFilerting ?? false ? filteredVehicles[selectedVehicleIndex] : vehicles[selectedVehicleIndex]}
    var numberOfRows: Int { return delegate?.isFilerting ?? false ? filteredVehicles.count : vehicles.count}
    ...
    func vehicle(at index: Int) -> Vehicle? {
        let vehicleSource = delegate?.isFilerting ?? false ? filteredVehicles : vehicles
